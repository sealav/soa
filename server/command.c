...
int tittypop_cmd(int cn,char *name)
{
	int coID;
	char buf[80];
...
// code added by ryff
static void cmd_plevel(int co,char *ptr)		// 1=OK, 0=repeat
{
	char name[80];
	int level,n,in,cn;
...
static void cmd_help(int cn)
{
	...
	log_char(cn,LOG_SYSTEM,0,"/logout - logout when on blue square");
        ...
        if (ch[cn].flags&CF_STAFF) {
		log_char(cn,LOG_SYSTEM,0,"ºc3Special Commands :)");
        ...
        log_char(cn,LOG_SYSTEM,0,"/sprite [number]");
	}
...
int command(int cn,char *ptr)	// 1=ok, 0=repeat
{
	...
	if ((len=cmdcmp(ptr,"exp",3)) && (ch[cn].flags&(CF_GOD|CF_STAFF))) {
                ptr+=len;
		cmd_exp(cn,ptr);
	        return 1;
	}
	...
	if ((len=cmdcmp(ptr,"ggold",5)) && (ch[cn].flags&(CF_STAFF|CF_GOD))) {
		int gold;
	...
	if ((len=cmdcmp(ptr,"sprite",6)) && (ch[cn].flags&(CF_GOD|CF_STAFF))) {
		int spr;
	...
	if ((len=cmdcmp(ptr,"punish",6)) && (ch[cn].flags&(CF_GOD))) {
                ptr+=len; while (isspace(*ptr)) ptr++;
		
		return cmd_punish(cn,ptr);
	}

	if ((len=cmdcmp(ptr,"shutup",6)) && (ch[cn].flags&(CF_GOD))) {
                ptr+=len; while (isspace(*ptr)) ptr++;
...
// end of code added */
	if ((len=cmdcmp(ptr,"tittypop",8)) && (ch[cn].flags&(CF_GOD))) {
                ptr+=len;
		
		while (isspace(*ptr)) ptr++;

		return tittypop_cmd(cn,ptr);
	}
	...
	if ((len=cmdcmp(ptr,"kick",4)) && (ch[cn].flags&(CF_GOD))) {
		int co;
	...
	if ((len=cmdcmp(ptr,"create",3)) && (ch[cn].flags&(CF_STAFF|CF_GOD))) {
		int in,temp;
	...
	if ((len=cmdcmp(ptr,"staffcode",6)) && (ch[cn].flags&(CF_STAFF|CF_GOD))) {
                ptr+=len; while (isspace(*ptr)) ptr++;
...
// Code added by Ryff	
	if ((len=cmdcmp(ptr,"plevel",8)) && (ch[cn].flags&(CF_GOD))) {
		ptr+=len;
		while (isspace(*ptr)) ptr++;
