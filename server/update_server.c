...
void rec_chat(int nr){
	int len = recv(chat[nr].sock,(char*)chat[nr].inbuf+chat[nr].in_len, \
	...
	while (!quit){
		...
		for (n=1; n < MAXCHAT; n++){
			if (chat[n].sock){
				...
				if (chat[n].handle != -1 && chat[n].iptr==chat[n].optr){
					send_block(n);
				}

				//printf("handle=%d, len=%d\n",chat[n].handle,chat[n].in_len);
				if (chat[n].handle == -1 && chat[n].in_len){
					parse_cmd(n);
				}
