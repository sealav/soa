...
int mysql_query_con(MYSQL *my,const char *query)
{
	...
	while (1) {
		...
		err=mysql_errno(my);
			...
			sleep(1);
			makemysqlpass();
			mysql_real_connect(my,"localhost","soagame","123456","mysql",0,NULL,0);
			destroymysqlpass();
...
int init_database(void)
{
	...
	if (!mysql_real_connect(&mysql,"localhost","soagame","123456","mysql",0,NULL,0)) {
                xlog("Connect to database failed.");
				xlog("This one doesn't connect :S.");
                exit(0);
	} else {
